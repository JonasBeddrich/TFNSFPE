...
int main(int argc, char *argv[]){
    ...
    pd = new ParaViewDataCollection("dt_a_psi_F0_" 
        + scenario
        + symmetry 
    ...
    for (int ti = 0; !done; ){
        ...
        tmp_block_vector.Add(w_inf, phi_Fpsi0_true_block); 
        tmp_block_vector.Add(w_inf * std::pow(t+dt, -alpha), phi_psi0_true_block); 

        // solve for psi^n+0.5 
        css.solve_Id_minus_theta_FR(tmp_block_vector, phi_psi_true_block); 
        
        // calculate M^-1 FR psi 
        ...
        for (int l = 0; l < n_modes; l++){
            ...
            phi_psi_true_modes[l].Add(dt * weights[l], phi_Fpsi0_true_block);
            phi_psi_true_modes[l].Add(dt * weights[l] * std::pow(t+dt, -alpha), phi_psi0_true_block);
